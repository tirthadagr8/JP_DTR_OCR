...
class DTrOCRModel(nn.Module):
    ...
    def initialise_weights(self, config: DTrOCRConfig) -> None:
        ...
        # Token embeddings: copy existing weights and initialize new weights
        pretrained_embedding_weights = pretrained_gpt2.wte.weight
        new_embedding_weights = self.token_embedding.weight

        with torch.no_grad():
            # Copy over weights for the overlapping part
            new_embedding_weights[:pretrained_embedding_weights.size(0)] = pretrained_embedding_weights

            # Initialize the rest of the new embeddings randomly
            nn.init.normal_(new_embedding_weights[pretrained_embedding_weights.size(0):], mean=0.0, std=0.02)

        # Reassign the modified embeddings
        self.token_embedding.weight = nn.Parameter(new_embedding_weights)
